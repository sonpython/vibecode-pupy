...
2. Live USB hardware probe via esptool: confirmed ESP32-S3 N16R8, 16MB flash + 8MB PSRAM, native USB-JTAG, MAC `<device-mac>`, currently running xiaozhi-esp32 v2.0.8 (ESP-IDF v5.5)
...
6. Full 16MB flash backup at `<factory-backup-path>` (SHA256 <factory-backup-sha256>)
...
- `<factory-backup-path>` — NEW (16MiB flash dump)
...
- Architecture split: Claude collector on Mac (launchd), Codex collector on Docker @ <docker-host-ip>, Usage-API on same Docker, exposed via Cloudflare Tunnel HTTPS.
